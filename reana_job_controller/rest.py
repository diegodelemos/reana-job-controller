...
from reana_job_controller.k8s import instantiate_job
...
@restapi_blueprint.route('/jobs', methods=['GET'])
def get_jobs():  # noqa
...
@restapi_blueprint.route('/jobs', methods=['POST'])
def create_job():  # noqa
    r"""Create a new job.
...
@restapi_blueprint.route('/jobs/<job_id>', methods=['GET'])
...
@restapi_blueprint.route('/jobs/<job_id>/logs', methods=['GET'])
def get_logs(job_id):  # noqa
    ...
    """
    if job_exists(job_id):
        return retrieve_job_logs(job_id), 200
    else:
        return jsonify({'message': 'The job {} doesn\'t exist'
                        .format(job_id)}), 404


@restapi_blueprint.route('/apispec', methods=['GET'])
